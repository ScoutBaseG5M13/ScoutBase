...
class FindAllStatsUseCaseTest {
    ...
    void execute_ShouldReturnAllStats() {
        List<Stat> stats = List.of(
                Stat.builder().id(UUID.randomUUID()).playerId(UUID.randomUUID()).code(StatEnum.MED.statCode).value(80).build(),
                Stat.builder().id(UUID.randomUUID()).playerId(UUID.randomUUID()).code(StatEnum.VEL.statCode).value(90).build()
        );
