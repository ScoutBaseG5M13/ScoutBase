...
class StatRepositoryImplIT {
    ...
    void shouldUpdateExistingStat() {
        ...
        Stat stat = new Stat(statId, playerId, StatEnum.AGR.statCode, 5);
        statRepository.save(stat);

        entityManager.flush();
        entityManager.clear();

        Stat updatedStat = new Stat(statId, playerId, StatEnum.CLI.statCode, 9);
    ...
    void shouldDeleteStat() {
        UUID statId = UUID.randomUUID();
        Stat stat = new Stat(statId, UUID.randomUUID(), StatEnum.COM.statCode, 6);
        statRepository.save(stat);
